...
def get_args():
    ...
    parser.add_argument('--n_iter', type=int, default=20, help='EM iterations')
    parser.add_argument('--sinkhorn_iter', type=int, default=3, help='Sinkhorn iterations')
    ...
    parser.add_argument(
        ...
        choices=[4, 5, 6],
        help=(
            '4 = Balanced OT + standard K-Means++ init, '
            '5 = Unbalanced OT + standard K-Means++ init, '
            '6 = Unbalanced OT + mass-weighted K-Means++ init'
        )
    )

    # 평가 설정
    parser.add_argument('--ndcg_k', type=int, default=5)
    parser.add_argument('--recall_k', type=int, default=1)
...
def main():
    ...
    logger.log(f"Ablation Type: {args.ablation_type}")

    for dataset_name in args.datasets:
        file_path = f'{args.data_dir}/{args.model}_{dataset_name}.npz'

        if not os.path.exists(file_path):
            logger.log(f"Skipping {dataset_name}: File not found at {file_path}")
            continue
        ...
        if 'attention' in docs_data:
            ...
            logger.log("Warning: 'attention' key not found. Using uniform attention.")
        ...
        targets = list(queries_data['relevant_doc_indices'])
        #targets = list(queries_data['relevant_doc_rels'])
        ...
        logger.log(f"Documents: {num_docs}, Max doc len: {max_doc_len}, Embed dim: {embed_dim}")

        for mf in args.mfs:
            num_clusters = max(1, int(max_doc_len // mf))

            logger.log(f"  > Processing MF={mf} | K={num_clusters}")
            ...
            if 'mmlongbench' in dataset_name:
                num_queries, num_docs = scores_numpy.shape
                candidate_mask = np.zeros((num_queries, num_docs), dtype=bool)

                for qi, qid in enumerate(q_ids):
                    query_pdf = qid.rsplit("::", 1)[0]
                    for di, docid in enumerate(doc_ids):
                        doc_pdf = docid.rsplit("::", 1)[0]
                        if doc_pdf == query_pdf:
                            candidate_mask[qi, di] = True

                masked_scores = scores_numpy.copy()
                masked_scores[~candidate_mask] = -1e9

                relevant_doc_indices = queries_data['relevant_doc_indices']

                # single relevant 기준
                ndcg10 = ndcg_k(masked_scores, relevant_doc_indices, 10)
                ndcg_score = ndcg_k(masked_scores, relevant_doc_indices, args.ndcg_k)
                recall_score = recall_at_k(masked_scores, relevant_doc_indices, args.recall_k)
            else: 
                ndcg10 = ndcg_k(
                        scores_numpy,
                        targets,
            ...
            logger.log(
                f"    Done. "
                f"nDCG@10: {ndcg10:.4f} | "
                f"nDCG@{args.ndcg_k}: {ndcg_score:.4f} | "
                f"Recall@{args.recall_k}: {recall_score:.4f} | "
                f"Latency: {latency_ms:.2f} ms/query | "
